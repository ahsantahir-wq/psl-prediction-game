...
function PredictionModal({ match, onClose, onSuccess }) {
    ...
    const handleSubmitPrediction = async ()=>{
        ...
        try {
            ...
            const { data: wallet } = await supabase.from('wallet').select('credits_balance').eq('user_id', user.id).single();
            ...
            // Update user stats
            // Update user stats
            const { data: currentStats } = await supabase.from('user_stats').select('total_predictions, credits_spent').eq('user_id', user.id).single();
            if (currentStats) {
...
function Dashboard() {
    ...
    const loadDashboard = async ()=>{
        try {
            ...
            const { data: matchesData } = await supabase.from('matches').select('*').in('status', [
                'upcoming',
                'live'
            ]).order('match_date', {
                ascending: true
...
//# sourceMappingURL=%5Broot-of-the-server%5D__1517bc71._.js.map